...
def detect_anomalies(df: pd.DataFrame) -> list:
    ...
    for col in numeric_cols[:8]:
        ...
        if len(outliers) > 0:
            ...
            alerts.append(f"**{col}**: {len(outliers)} outliers ({pct}%) detected outside 2.5xIQR range [{round(q1-2.5*iqr,2)}, {round(q3+2.5*iqr,2)}]")
    return alerts


def build_chart(df: pd.DataFrame, chart_type: str, x: str, y: str, color: str = None, title: str = ""):
    try:
        df = df.copy()
...
def render_header():
    ...
    <div class="brand-header">
      ...
      <p class="brand-subtitle">AI Dashboard Builder &nbsp;&middot;&nbsp; Natural Language Queries &nbsp;&middot;&nbsp; Auto Data Cleaning &nbsp;&middot;&nbsp; PDF &amp; PowerPoint Export</p>
    </div>
    """, unsafe_allow_html=True)

    b = st.session_state
    def badge(label, done):
        ...
        for rec in st.session_state.recommendations:
            ...
            <div class="info-card">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.4rem">
                ...
                <span style="font-size:0.75rem;color:{color};font-weight:700;font-family:'Space Grotesk',sans-serif">{impact} IMPACT &middot; {rec.get('timeline','')}</span>
              </div>
              <p>{rec.get('detail','')}</p>
            </div>
            """, unsafe_allow_html=True)
...
[
  {{"type": "<chart_type>", "x": "<col>", "y": "<col>", "color": "<col or null>", "title": "<title>"}},
  ...
]
    ...
    if st.session_state.snapshots:
        for snap in reversed(st.session_state.snapshots):
            ...
            <div class="info-card">
              ...
              <p style="margin:0.3rem 0 0 0">{snap['rows']:,} rows &middot; {len(snap['kpis'])} KPIs tracked &middot; {len(snap['anomalies'])} anomalies</p>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown("""
